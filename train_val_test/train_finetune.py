...
with TimerBlock("Good Luck") as block:
    ...
    optimizer = optimizer_choose(model, args, block)
    ...
    if args.val_first:
        model.eval()
        loss, acc, score_dict, all_pre_true, wrong_path_pre_true  = val_net(data_loader_val, model, loss_function, global_step, args)
    ...
    for epoch in process:
        ...
        global_step, loss = train_net(data_loader_train, model, loss_function, optimizer, global_step, args)
        block.log('Training finished for epoch {}, LR: {}, LOSS: {:4f}'.format(epoch, lr, loss))
        model.eval()
        loss, acc, score_dict, all_pre_true, wrong_path_pre_true = val_net(data_loader_val, model, loss_function, global_step, args)
    ...
    torch.save(save, args.model_saved_name + '-' + str(epoch) + '-' + str(global_step) + '.state')
    # block.log('Best model: ' + args.model_saved_name + '-' + str(best_epoch) + '-' + str(best_step) + '.state, acc: ' + str(
    #         best_accu))

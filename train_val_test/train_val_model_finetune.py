...
def train_classifier(data_loader, model, loss_function, optimizer, global_step, args):
    ...
    for index, (inputs, inputs1, labels) in enumerate(process):
        ...
        _, _, _, _, _, _, _, _, _, _, y1, y2 = model(inputs, inputs1)
        y1 = model.module.fc(y1)
        y2 = model.module.fc(y2)
        # y3 = model.module.fc(y3)
        # out1s = model.module.fc(out1s.mean(1))
        # out1t = model.module.fc(out1t.mean(1))
        # out2s = model.module.fc(out2s.mean(1))
        # out2t = model.module.fc(out2t.mean(1))
        # out3s = model.module.fc(out3s.mean(1))
        # out3t = model.module.fc(out3t.mean(1))

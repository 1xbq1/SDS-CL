...
class info_nce_loss(nn.Module):
    ...
    def forward(self, features, shape0):
        N = features.shape[0]
        labels = torch.cat([torch.arange(shape0) for i in range(self.n_views)], dim=0)
        labels = (labels.unsqueeze(0) == labels.unsqueeze(1)).float()
        # print(labels.unsqueeze(0).shape)
        labels = labels.unsqueeze(0).expand(N, -1, -1).cuda()

        features = F.normalize(features, dim=2)

        similarity_matrix = torch.matmul(features, features.transpose(1,2)) #N,2V,2V or N,2T,2T
        # assert similarity_matrix.shape == (
        #     self.args.n_views * self.args.batch_size, self.args.n_views * self.args.batch_size)
        # assert similarity_matrix.shape == labels.shape

        # discard the main diagonal from both: labels and similarities matrix
        mask = torch.eye(labels.shape[1], dtype=torch.bool).unsqueeze(0).expand(N, -1, -1).cuda()
        labels = labels[~mask].view(N, labels.shape[1], -1)
        similarity_matrix = similarity_matrix[~mask].view(N, similarity_matrix.shape[1], -1)
        # assert similarity_matrix.shape == labels.shape

        # select and combine multiple positives
        positives = similarity_matrix[labels.bool()].view(N, labels.shape[1], -1)

        # select only the negatives the negatives
        negatives = similarity_matrix[~labels.bool()].view(N, similarity_matrix.shape[1], -1)
